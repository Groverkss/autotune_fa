...
module attributes { transform.with_named_sequence } {
  ...
  transform.named_sequence @__attention_main_len_{{ head_dim }}(%variant_op: !transform.any_op {transform.consumed}) {
    ...
    transform.structured.tile_using_forall %attention tile_sizes [1, {{ block_m }}]
    ...
    %acc_fill, %max_fill, %sum_fill, %inner_loop, %final_scaling, %last_truncate, %blocked_attention = transform.iree.tile_attention %attention4 {tile_size = {{ block_n }}} :
    ...
    %scale_q, %fill_op, %first_matmul, %reduce_max, %partial_softmax, %scale_factor, %update, %reduce_sum, %truncate, %scale_acc, %second_matmul
        = transform.iree.decompose_tiled_attention %blocked_attention {tile_size = {{ block_n }}} :
    ...
    %tiled_fill, %fill_grid = transform.structured.tile_using_forall %fills tile_sizes[{{ block_m // num_warps }}] (mapping = [#gpu.warp<linear_dim_0>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)

    // Distribute last_truncate and fuse final_scaling into it
    // ==========================================
    %tiled_truncate, %loop_truncate = transform.structured.tile_using_forall %last_truncate tile_sizes[{{ block_m // num_warps }}] (mapping = [#gpu.warp<linear_dim_0>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
