...
class Runner:
    ...
    def __init__(self, tuner_config: TunerConfig, kernel_config: KernelConfig, runner_config: RunnerConfig):
    ...
    def compile(self, input_file: Path):
        ...
        flags += [f'-iree-hal-benchmark-dispatch-repeat-count={self.runner_config.iree_benchmark_reps}']
        flags += [
          f'{str(input_file)}',
          '-o',
          self.get_vmfb_file()
    ...
    def compute_reference_inputs_and_outputs(self):
        ...
        with open(f'value_{self.get_shape_data(self.tuner_config.transpose_v)}.npy', 'wb') as f:
    ...
    def run(self):
        ...
        def evaluate():
            nonlocal self
            if state == COMPILE: self.compile(input_file)
            if state == VALIDATE: self.validate()
            if state == BENCHMARK: self.benchmark()

        def log():
            if state == COMPILE: logger.info("Compiling ...")
            if state == VALIDATE: logger.info("Validating ...")
            if state == BENCHMARK: logger.info("Benchmarking ...")
...
if __name__ == "__main__":
    tuner_config = TunerConfig(1, 20, 4096, 64, "gfx940", Path("spec.mlir"), False, Path("./tmp"), Path("../iree/build"))
